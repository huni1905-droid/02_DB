 ...
 * 
 * - 데이터를 조회(SELECT)하면 조건에 맍는 행들이 조회됨.
...
-- WHERE : 조건을 따지는 구문
-- 문자열('1')이어도 숫자타입(1)이면 숫자열(1)로 인식한다

-- 문자열 타입이어도 저장된 값이 숫자면 자동으로 형변환되어 연산 가능
--EMP_ID는 CARCHAR2 타입 즉 문자열 타입이지만 저장된 값이 숫자라서 연산 가능
...
SELECT SYSDATE -1, SYSDATE, SYSDATE + 1 
FROM DUAL;
--컬럼명이 : SYSDATE -1, SYSDATE, SYSDATE 로 뜸
...
-- JAVA 리터럴 : 값 자체를 의미 ex) long number = 222L;
-- DB 리터럴 : 임의로 지정한 겂을 기존 테이블에 존재하는 값처럼 사용하는 것
--> DB의 리터럴 표기법 '' 홑따옴표
...
-- 4. OREDR BY 컬럼명 | 별칭 | 컬럼 순서 [ASC(오름차순) | DESC(내림차순)] [NULLS FIRST | LAST]
-- (해석 순서- 1. 2. 3. 4.)

-- EMPLOYEE 테이블에서 (FROM 절)
-- 급여가 3맥만원 초과인 사원의 (WHERE 절)
...
-- EMPLOYEE 테이블에서 전화번호가 010으로 시작하지 않는 사원 사번, 이름, 전화번호 조회
SELECT EMP_ID, EMP_NAME, PHONE
FROM EMPLOYEE
WHERE PHONE NOT LIKE '010%';

-- EMPLOYEE 테이블에서 EMAIL의 _ 앞에 글자가 세글자인 사원만 이름, 이메일 조회
SELECT * FROM EMPLOYEE;
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE NOT IN('D1', 'D6', 'D9') --12명 => NULL 값인 사람 안 나와
OR DEPT_CODE IS NULL;-- NULL 포함해서 나오게 하려면 
...
-- EMPLOYEE 테이블에서 보너스가 있는 사원의 이르, 보너스 조회
...
ORDER BY 3 DESC; -- 조회되는 RESULT SET의 컬럼 순서의 3번 컬럼을 기준으로 정렬하겠다.


-- 입사일 순서대로 이름, 입사일 조회 (별칭 사용)
SELECT EMP_NAME 이름, HIRE_DATE 입사일 -- 별칭 지어주고
FROM EMPLOYEE
ORDER BY 입사일; -- 해석 우선순위상 SELECT절 이후에 해당하는 ORDER BY절 에서는 별칭 사용이 가능하다

-- 정렬 중첩 : 대분류 정렬 후 소분류 정렬
-- 부서코드 오름차순 정렬 후 급여 내림차순 정렬
SELECT EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE
ORDER BY DEPT_CODE, SALARY DESC;-- 정렬 순서에 따라 ORDER BY 이후에 순서대로 나열해주면 된다 / 오름차순이 기본 값이라서 

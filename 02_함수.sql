...
SELECT INSTR('AABAACAABBAA', 'B') FROM DUAL; -- 3(첫번째 문자 순번 1부터 시작)
...
-- EMPLOYEE 테이블에서 사원의 월급을 100만으로 나눴을떄 나머지 값 조회
...
SELECT SYSTIMESTAMP FROM DUAL; --2025-10-20 09:16:21.880 +0900

-- MONTHS_BETWEEN(날짜, 날짜) : 두 날짜의 개월 수 차이 반환
SELECT ABS(ROUND(MONTHS_BETWEEN(SYSDATE, '2026-02-27'), 1)) "수강 기간(개월)" 
FROM DUAL;-- -4.21326164874551971326164874551971326165 -> -4.2 -> 4.2
...
-- ADD_MANTH(날짜, 숫자) : 날짜에 숫자만큼의 개월 수를 더함.(음수도 가능)
SELECT ADD_MONTHS(SYSDATE, 4) FROM DUAL; -- 2026-02-20 09:30:13.000

SELECT ADD_MONTHS(SYSDATE, -1) FROM DUAL; -- 2025-09-20 09:31:04.000

-- LAST_DAY(날짜) : 해당 달의 마지막 날짜를 구함
SELECT LAST_DAY(SYSDATE) FROM DUAL; --2025-10-31 09:33:01.000
...
SELECT TO_CHAR(1234) FROM DUAL; -- 문자열 타입으로 찍으면 1234
SELECT 1234 FROM DUAL; -- 숫자로 찍으면 1,234
...
FROM DUAL; --2025/10/20 10:08:26 월요일

-- 10/20 (월)
SELECT TO_CHAR(SYSDATE, 'MM/DD (DY)') FROM DUAL;

-- 2025년 10월 20일 (월)
SELECT TO_CHAR(SYSDATE, 'YYYY"월" MM"월" DD"일" (DY)') FROM DUAL;
--ORA-01821: 날짜 형식이 부적합합니다 --> 2025월 10월 20일 (월)
...
-- R 패턴 : 1세기 기준으로 절반(50년) 이상인 경우는 어전 세기(1900년대)
...
-- EMPLOY 테이블에서 각 직원이 태어난 생년월일 조회
-- 사원이름, 생년월일(1965년 10월 80일)
SELECT * FROM  EMPLOYEE;
...
SELECT '1,000,000' + 50000 FROM DUAL; --ORA-01722: 수치가 부적합합니다 (, 때문에)
SELECT '1000000' + 50000 FROM DUAL; --이것은 가능
...
-- NVL2(컬럼명, 바꿀값1, 바꿀값2)
-- 해당 컬럼의 값이 있으면 바꿀값1으로 변경, NULL이면 바꿀값2로 변경

--EMPLOYEE 테이블에서 보너스를 받으면 '0', 안 받으면 'X' 조회
SELECT EMP_NAME, NVL2(BONUS, '0', 'X') "보너스 수령" FROM EMPLOYEE;
...
-- DECODE(계산식 | 컬럼명, 
--  조건값1, 선택값1, 조건값2, 선택값2,......, 아무것도 일치하지 않을 때)
...
/*3*/SELECT SUM(SALARY), ROUND(AVE(SALARY))
/*1*/FROM EMPLOYEE
...
-- 급여 최대값, 가장늦은 입사일, 알파벳순서가 가장 늦은 이메일 조회

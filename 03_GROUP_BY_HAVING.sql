...
-- 1) 부서코드만 조회
SELECT DEPT_CODE FROM EMPLOYEE; -- 23개 행

-- 2) 급여 합 조회
SELECT SUM(SALARY) FROM EMPLOYEE; -- 1개 행
...
SUM(SALARY) "급여 합"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO, 8, 1), '1', '남', '2', '여')  -- 별칭 사용X (아직 SELECT 절 해석X )
...
-- * 여럼 컬럼을 묶어서 그룹으로 지정 가능 -> 그룹 내 그룹이 가능하다! *
...
FROM EMPLOYEE
GROUP BY DEPT_CODE 
-- WHERE SALARY >= 3000000 -- 한 사람의 급여가 300만원 이상이라는 조건(문제의 요구사항X)
HAVING AVG(SALARY) >=3000000 -- DEPT_CODE 그룹 중 급여 평균이 300만원 이상인 그룹만 조회(문제의 요구사항O)
ORDER BY DEPT_CODE;
-- WHERE : 각각 독립된 컬럼에서 따질때 써줘
-- HAVING : GROUP BY 절에서 그룹화한 그룸 내에서 적용할 조건
...
ORDER BY 1; -- 정렬은 컬럼 순서를 써서 정렬 가능 
...
--UNION ALL
MINUS
-- 급여가 300만원 초가인 사원의 사번, 이름, 부서코드, 급여 조회
SELECT EMP_ID, EMP_NAME, DEPT_CODE, SALARY
FROM EMPLOYEE

...
FROM EMPLOYEE 							-- 이 테이블을 기준으로(SELECT할 컬럼이 더 많으니깐) 
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);-- DEPARTMENT 을 조인하겠다   
										--	EMPLOYEE 테이블의 DEPT_CODE과 DEPARTMENT 테이블의 DEPT_ID 가 같으니깐 
										--  이를 연결고리 삼아서 JOIN(공유하고 있는 컬럼이 있는지 확인하고 연결)

-- 1. 내부조인(INNER JOIN) (== 등가조인(EQUAL JOIN)) 
--> 연결되는 컬럼의 값이 일치하는 행들만 조인됨.
--> 일치하는 값이 없는 행은 조인에서 제외됨.(EX. NULL일떄)
...
WHERE DEPT_CODE = DEPT_ID; -- JOIN절이 아니라 WHERE절 사용!
...
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE); -- 컬럼명이 같으니깐 =로 안 묶어줘도 된다-------------------------------------------추가(ON을 이용한 코드)----------------

-- 오라클 구문
SELECT EMP_ID, EMP_NAME, E.JOB_CODE, JOB_NAME -- SELECT절에도 테이블명(OR 별칭)의 별칭 줘야 한다
FROM EMPLOYEE E, JOB J
WHERE E.JOB_CODE = J.JOB_CODE;
--ORA-00918: 열의 정의가 애매합니다 (WHERE JOB_CODE = JOB_CODE)
...
SELECT EMP_ID, EMP_NAME, DEPT_CODE, DEPT_TITLE
FROM EMPLOYEE 								
/*INNER*/ JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID); -- INNER 명시 안해도 INNER JOIN 으로 인식
												   

-- 1) LEFT [OUTER] JOIN : 합치기에 사용한 두 테이블 중 
--		왼편에 기술된 테이블의 컬럼수를 기준으로 JOIN한다. (OUTER 표기 생략 가능)
--> 왼편에 작성된 테이블의 모든 행이 결과에 포함되어야 한다.
--> (JOIN이 안 되는 행도 결과에 포함된다)

-- ANSI 표준
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE  
LEFT /*OUTER*/ JOIN DEPARTMENT -- 순서가 중요!!
ON(DEPT_CODE = DEPT_ID); -- 23행 (DEPT_CODE가 NULL인 하동운, 이오리 포함되서 나온다)

-- 오라클 구문
SELECT EMP_NAME, DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEMT_ID(+);
-- 기준 테이블이 아닌 반대쪽 테이블 컬럼에 (+) 기호 작성해야한다!
...
-- 조인되는 테이블의 각 행들이 모두 매팽된 데이터가 검색되는 방법 (곱집합)
...
-- 테이블마다 별칭 작성(별칭 미작성시 열의정의가 애매하다.. 오류 발생)
...
-- ANSI 표준
SELECT E1.EMP_NO 사번, E1.EMP_NAME 사원이름, 
		NVL(E1.MANAGER_ID, '없음') "사수의 사번", 
		NVL(E2.EMP_NAME, '-') "사수 이름" -- SELECT절에서 컬럼 전체의 테이블을 정의해줘야 한다!
		-- NVL : NULL이면 출력할 값 
FROM EMPLOYEE E1 
...
-- 오라클 구문
SELECT E1.EMP_NO 사번, E1.EMP_NAME 사원이름, 
		NVL(E1.MANAGER_ID, '없음') "사수의 사번", NVL(E2.EMP_NAME, '-')
...
--> NATURAL JOIN을 잘못 조회하면 CROSS JOIN 결과 조회된다.
...
-- EMPLOYEE와 LOCATION은 JOIN 불가능 하지만
-- EMPLOYEE와 DEPARTMENT JOIN 하고 추가적으로 LOCATION JOIN 한다
...
AND SUBSTR(EMP_NAME, 1, 1) = '전'; -- == EMP_NAME LIKE '전%'
...
WHERE DEPT_TITLE ='해외영업1부' OR DEPT_TITLE ='해외영업2부'; -- == WHERE DEPT_TITLE IN('해외영업1부', '해외영업2부')
														-- == WHERE DEPT_TITLE LIKE '%1부'OR WHERE DEPT_TITLE LIKE '%2부'


--*****4. 보너스포인트를 받는 직원들의 사원명, 보너스포인트, 부서명, 근무지역명을 조회하시오.*****
SELECT EMP_NAME, BONUS, DEPT_TITLE, LOCAL_NAME
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID) -- LEFT JOIN 해줘야 부서코드가 없는 하동운씨까지 나온다
...
-- 부서가 있는 사원만 조회하고 싶으면 
-- INNER JOIN 사용시 자동으로 부서가 없는 사람들 사라지므로 WHERE절 없어도 된다.


--6. 급여등급별 최소급여(MIN_SAL)를 초과해서 받는 직원들의 사원명, 직급명,급여, 연봉(보너스포함)을 조회하시오. (연봉에 보너스포인트를 적용하시오.)
SELECT EMP_NAME, JOB_NAME, SALARY, NVL(SALARY*(BONUS+1), SALARY*12) 연봉  -- SALARY *(1+NVL(BONUS, 0))*12
FROM EMPLOYEE
JOIN JOB USING (JOB_CODE) -- == NATURAL JOIN JOB 주의사항!
JOIN SAL_GRADE USING(SAL_LEVEL)
WHERE SALARY > MIN_SAL;

-- 주의사항!
-- NATURAL JOIN을 다중조인에서 사용할 때는 
-- SELECT 절에 NATURAL JOIN에서 사용되었을 연결고리 컬럼을 반드시 작성해야함!
SELECT EMP_NAME, JOB_NAME, SALARY, JOB_CODE/*주의*/, NVL(SALARY*(BONUS+1), SALARY*12) 연봉  -- SALARY *(1+NVL(BONUS, 0))*12
FROM EMPLOYEE 
NATURAL JOIN JOB --주의!
JOIN SAL_GRADE USING(SAL_LEVEL)
WHERE SALARY > MIN_SAL;
...
WHERE NATIONAL_NAME = '한국' OR NATIONAL_NAME = '일본'; -- NATIONAL_CODE IN('JP', 'KO')

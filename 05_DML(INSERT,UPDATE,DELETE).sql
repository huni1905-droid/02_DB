...
VALUES('900', '홍길도', '991215-1234567', 'hong_gd@or.kr', 
...
--  (DEFUALT 존재 시 DEFAULT 설정값으로 삽입됨) CF) 복사본은 DEFUALT 값 따라오지 않는다
...
SELECT * FROM EMPLOYEE2
WHERE EMP_ID ='900';
-- 조회 다시 해봄(영구저장 되었기 떄문에 롤백한다 하더라도 되돌리기 안된다)
...
FROM EMPLOYEE2 
LEFT JOIN DEPARTMENT2 ON (DEPT_CODE = DEPT_ID); -- 서브쿼리
...
SELECT EMP_ID, EMP_NAME, DEPT_TITLE
FROM EMPLOYEE2 
LEFT JOIN DEPARTMENT2 ON (DEPT_CODE = DEPT_ID)-- 위에서 만든 서브쿼리를 그대로 INSERT 구문 안으로 갖고 온다
);
...
WHERE EMP_NAME = '유재식'; -- 0.2 -- 이 둘을 서브쿼리로 이용
...
VALUES(999, '곽두원', '561016-1234567', 'kwack_dw@or.kr',
 '010111122222', 'D9', 'J4', 'S1', 9000000, NULL, NULL, SYSDATE, NULL, NULL);
...
MERGE INTO EMP_M01 USING EMP_M02 ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID) -- M01테이블을 기준으로 M02테이블을 병합
WHEN MATCHED THEN
UPDATE SET

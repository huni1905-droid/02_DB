...
-- FROMM JOIN으로 인해 테이블상 존재하는 컬럼이면 
-- ORDER BY 절 사용 가능!!!
...
SELECT MAX(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE; -- 7행 1열(다중행) - 서브쿼리의 결과값에 행의 갯수가 여러개
...
SELECT EMP_NAME, DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE IN (SELECT DEPT_ID		-- 다중행 서브쿼리 -> 일반비교가 아니라 IN 과 같은 연산자 쓴다
					FROM DEPARTMENT
					WHERE LOCATION_ID = (SELECT LOCAL_CODE
										FROM LOCATION
										WHERE NATIONAL_CODE = 'KO')); -- 단일행 서브쿼리여서 앞에 =와 같은 일반비교 사용
...
------------------------연습 문제----------------------------
-- 1. 노옹청 사원과 같은 부서, 같은 직급인 사원을 조회(단, 노옹철 제외)
-- 사번, 이름, 부서코드, /직급코드, 부서명, 직급명 조회ㅇ
SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE EMP_NAME = '노옹철'; 
...
SELECT DEPT_CODE, JOB_CODE
FROM EMPLOYEE
WHERE EXTRACT(YEAR FROM HIRE_DATE) = 2000; --인사관리부	사원
...
--3 77년생 여자 사원과 동일한 부서이면서 동일한 사수를 가지고 있는 사워 조회
...
WHERE HIRE_DATE = (SELECT MIN(HIRE_DATE)
					...
					-- 퇴사한 직원은 제외 조건을 메인 쿼리에 넣으면 
					-- D8의 부서의 입사일이 가장 빠른 사원인 전형돈이 속한 D8 전체의 결과사 사라지게 되어서 
					-- 서브쿼리에 퇴사한 직원은 제외 조건을 넣어줘야 한다.
...
FROM (SELECT EMP_NAME, SALARY
		FROM EMPLOYEE
		ORDER BY SALARY DESC) -- 해석순위1위 FROM 절에서 전체 직원 SALARY 내림차순 정렬
...
-- 9. RANK() OVER(정렬순서) / DENSE_RANK() OVER()

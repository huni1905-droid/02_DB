...
COMMIT;
INSERT INTO MEMBER VALUES('MEM02', 'QWER123', '김영희', '000123-2233445', SYSDATE);

--> 가입일 -> INSERT 시 미작성 하는 경우
-- DEFAULT 값이 반영되는지 확인
INSERT INTO MEMBER(MEMBER_ID, MEMBER_PWD, MEMBER_NAME) 
VALUES('MEM03', '1Q2W3E', '이지연'); -- SSN : NULL , ENROLL_DATE : DEFAULT로 지정한 SYSDATE로 들어감
...
CREATE TABLE USER_USED_NN(
	...
	GENDER VARCHAR2(10),		
	PHONE VARCHAR2(20),			
	EMAIL VARCHAR2(50)			-- <컬럼레벨>(마지막 컬럼 작성 부분까지가 컬럼 레벨이다)
	);-- <테이블 레벨>(컬럼레벨 아래)
...
-- ORA-01400: NULL을 ("KH_KJH"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- * 제약조건 지정 방법
-- 1) 컬럼 레벨 : [CONSTRAINT 제약조건명] 제약조건 종류
-- 2) 테이블 레벨 : [CONSTRAINT 제약조건명] 제약조건 종류(컬럼명)

-- UNIQUE 제약조건 테이블 생성
CREATE TABLE USER_USED_UK(
	USER_NO NUMBER NOT NULL,
	-- USER_ID VARCHAR2(20) UNIQUE, -- 컬럼레벨에서 제약조건 지정(제약조건명 미지정) 	
	-- USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- 컬럼레벨에서 제약조건 지정(제약조건명 지정)
	USER_ID VARCHAR2(20), -- UNIQUE 제약조건 설정된 컬럼(회원 아이디)
	USER_PWD VARCHAR2(20),		
	USER_NAME VARCHAR2(30),		
	GENDER VARCHAR2(10),		
	PHONE VARCHAR2(20),			
	EMAIL VARCHAR2(50), -- * 테이블레벨에서 제약조건등 지정하려면 마지막 컬럼레벨에 , 찍어야 한다
	-- <테이블 레벨>
	-- UNIQUE (USER_ID) -- 테이블레벨에서 제약조건 지정(제약조건명 미지정)
	CONSTRAINT UWER_ID_U UNIQUE(USER_ID) -- 테이블레벨에서 제약조건 지정(제약조건명 지정)
);
...
INSERT INTO USER_USED_UK VALUES
(1, 'USER01', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_KJH.UWER_ID_U)에 위배됩니다
...
INSERT INTO USER_USED_UK VALUES
(1, NULL, 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
--> 아이디에 NULL 값 중복 삽입 가능 확인 (UNIQUE 제약조건은 NULL 값 중복 허용)
...
CREATE TABLE USER_USED_UK2(
	...
	GENDER VARCHAR2(10),		
	PHONE VARCHAR2(20),			
	EMAIL VARCHAR2(50), -- 테이블 레벨에서 지정 하려면 마지막 컬럼레벨에 , 있어야 하는것 항상 CHECK!
...
INSERT INTO USER_USED_UK2 VALUES
(1, 'USER02', 'PASS01', '고길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_KJH.USER_ID_NAME_U)에 위배됩니다
--> UNIQUE 복합키로 지정된 컬럼값 중 하나라도 다르면 위배되지 않음
--> 모든 컬럼의 값이 중복되면 위배된다!
...
CREATE TABLE USER_USED_PK(
	USER_NO NUMBER CONSTRAINT USER_NO_PK PRIMARY KEY, -- 컬럼 레벨(에 제약조건명 지정)
	USER_ID VARCHAR2(20), 		
	USER_PWD VARCHAR2(20),		
	USER_NAME VARCHAR2(30),		
	GENDER VARCHAR2(10),		
...
INSERT INTO USER_USED_PK2 VALUES
(2, 'USER02', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-00001: 무결성 제약 조건(KH_KJH.PK_USERNO_USERID)에 위배됩니다
--> USER_NO, USER_ID 둘 다 중복되었을 때만 제약조건 위배!

INSERT INTO USER_USED_PK2 VALUES
(NULL, 'USER02', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-01400: NULL을 ("KH_KJH"."USER_USED_PK2"."USER_NO") 안에 삽입할 수 없습니다

INSERT INTO USER_USED_PK2 VALUES
(3, NULL, 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-01400: NULL을 ("KH_KJH"."USER_USED_PK2"."USER_ID") 안에 삽입할 수 없습니다
--> NULL 값은 둘 중 하나라도 NULL 이면 위배!(에러 발생)
...
-- 컬럼레벨일 경우 
-- 컬럼명 자료형(크기) [CONSTRAINT 이름] REFERENCES 참조할 테이블명 [(참조할 컬럼명)] [삭제룰]




-- 테이블레벨일 경우
-- [CONSTRAINT 이름] FOREIGN KEY (적용할 컬럼명) REFERENCES 참조할 테이블명 [(참조할 컬럼명)] [삭제룰]
...
CREATE TABLE USER_GRADE(
	GRADE_CODE NUMBER PRIMARY KEY,		-- 틍급 고유 식별 번호
...
INSERT INTO USER_USED_FK VALUES
(5, 'USER05', 'PASS05', '윤봉길', '남자', '010-6666-7777', 'yoon@kr.or.kr', 50);
-- ORA-02291: 무결성 제약조건(KH_KJH.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
...
-- ORA-02292: 무결성 제약조건(KH_KJH.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
SELECT * FROM USER_USED_FK2; -- 10을 사용하고 있던 자식값이 NULL이 됨을 확인
...
SELECT * FROM USER_USED_FK3; -- 부모의 10을 참조하고 있던 자식 행 삭제됨을 확인.
...
(1, 'USER01', 'PASS01', '홍길동', '남자', '010-1234-5678', 'hong@kr.or.kr');
-- ORA-02290: 체크 제약조건(KH_KJH.GENDER_CHECK)이 위배되었습니다
...
INSERT INTO USER_USED_CHECK VALUES
(2, 'USER02', 'PASS02', '유관순', '여자', '010-1111-3333', 'yoo@kr.or.kr');
-- ORA-02290: 체크 제약조건(KH_KJH.GENDER_CHECK)이 위배되었습니다

INSERT INTO USER_USED_CHECK VALUES
(2, 'USER02', 'PASS02', '유관순', '여', '010-1111-3333', 'yoo@kr.or.kr');
-- 가능

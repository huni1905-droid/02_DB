 ...
 * ALTER(바꾸다, 수정하다, 변조하다)
 * 
 * -- 테이블에서 수정할 수(바꿀수) 있는 것
 * 1) 제약 조건(추가/삭제) 	* 이미 만들어진 제약조건 자체를 수정하는 구문은 별도 존재하지 않음
 * 					  	* 수정 필요시 삭제 후 새로 추가해야함
 * 2) 컬럼(추가/수정/삭제)
...
-- DEPARTMENT 테이블 복사 (컬럼명, 데이터타입, NOT NULL 만 복사된다- PK, COMMENT, DEFAULT등은 복사 안 된다)
...
-- 컬럼 자제에 NULL 허용/비허용을 제어하는 성질 변경의 형태로 인식됨.
...
-- ORA-12899: "KH_KJH"."DEPT_COPY"."DEPT_ID" 열에 대한 값이 너무 큼(실제: 3, 최대값: 2)
--> DEPT_ID의 테이터 타입이 CHAR(2) 이므로 영어+숫자 2글자까지 저장 가능
--> D10 는 영어+숫자 3글자!!!
--> VARCHAR2(3)으로 변경해보기 (남는 바이트 메모리 반환을 위해 = 메모리 낭비를 방지) 
...
--> 테이블 자체가 삭제되는 결과가 나타나기 때문에 마지막 컬럼은 삭제할수 없다고 뜬다
...
SELECT * FROM SELECT * FROM DEPT_COPY;
...
-- 부모인 TB1 테이블 삭제하려고 할때 
DROP TABLE TB1;
-- ORA-02449: 외래 키에 의해 참조되는 고유/기본 키가 테이블에 있습니다
--> 해결 방법
-- 1) 자식테이블, 부모테이블 순서로 삭제
-- 2) ALTER 를 이용해서 FK 제약조건 삭제 후 부모테이블 삭제
...
SELECT * FROM TB2; -- TB2 테이블은 독립적인 테이블로 혼자 남게됨(아무 관계 없음)
...
-- 		CF) DML(INSERT/UPDATE/DELETE) 만이 TCL(COMMIT / ROLLBACK) 의 대상이다.
...
SELECT * FROM TB2;
-- 롤백 안 된다. 
-- 위에서 DDL 구문 중 ALTER를 사용하여 그 시점에 COMMIT이 되었기 때문이다.

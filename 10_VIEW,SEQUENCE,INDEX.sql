 ...
 * 
 * 
 *  ** VIEW 작성법 ** -- VIEW 도 객체이기 때문에 CREATE 구문으로 생성
 ...
 * 
 *  1) OR REPLACE 옵션*** : 
 * 		기존에 동일한 이름의 VIEW가 존재하면 이를 변경(덮어 쓰겠다)
 * 		없으면 새로 생성
 * 
 *  2) FORCE | NOFORCE 옵션 : 
...
ORDER BY 사번;
-- ORA-00955: 기존의 객체가 이름을 사용하고 있습니다. -> OR REPLACE 써주기
...
SELECT * FROM V_EMP
WHERE 직급명 = '대리' -- JOB_NAME 이 아니라 직급명으로 조회해야 한다.
ORDER BY 이름;
...
-- VIEW 생성 시 사용한 원본테이블 DEPT_COPY2에 값이 INSERT 됨을 확인
 ...
 * 
 * 2) 시퀀스명.CURRVAL : 현재 시퀀스 번호를 얻어옴.(= CURRENT VALUE)
...
NOMINVALUE 		-- 최소값 없음
NOCYCLE 		-- 반복 안함 (150까지 찍으면 그 다음부터는 오류 발생)
...
-- 시퀀스 생성 후 첫 NEXTVAL == START WITH 값으로 지정한 100이 나온다

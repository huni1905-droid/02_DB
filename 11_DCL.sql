...
-- CREATE TABLE : 테이블 생성권한이 없다.(아직 접속 및 로그인 권한만 있어)
...
ALTER USER kjh_sample DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;
-- SYSTEM 테이블 스페이스를 사용자 kjh_sample의 저장소로 무제한 사용 가능하게끔 하는 명령


-- 7. (kjh_sample) 테이블 다시 생성
CREATE TABLE TB_TEST(
	PK_COL NUMBER PRIMARY KEY,
	CONTENT VARCHAR2(30)
);
...
-- 1. (sample) kh_kjh 계정의 EMPLOYEE 조회
SELECT * FROM kh_kjh.EMPLOYEE;
-- ORA-00942: 테이블 또는 뷰가 존재하지 않습니다
-- sample 계정은 kh 계정의 테이블에 접근 권한이 없어서 조회 불가
...
-- 5. (sample) kh 계정의 EMPLOYEE 테이블 조회 불가능한지 다시 확인
SELECT * FROM kh_kjh.EMPLOYEE;
--> 조회 잘 됨을 확인. 
-- 조회 불가능 (권한 회수 완료)
